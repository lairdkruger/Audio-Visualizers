...
THREE.GPUParticleSystem = function(options) {
    ...
    this.particleNoiseTex =
        this.PARTICLE_NOISE_TEXTURE || textureLoader.load('textures/perlin-512.png')
    this.particleNoiseTex.wrapS = this.particleNoiseTex.wrapT = THREE.RepeatWrapping

    this.particleSpriteTex =
        this.PARTICLE_SPRITE_TEXTURE || textureLoader.load('textures/particle2.png')
...
THREE.GPUParticleContainer = function(maxParticles, particleSystem) {
    ...
    this.particleShaderGeo.addAttribute(
        'startTime',
        new THREE.BufferAttribute(new Float32Array(this.PARTICLE_COUNT), 1).setDynamic(
            true
        )
    )
    this.particleShaderGeo.addAttribute(
